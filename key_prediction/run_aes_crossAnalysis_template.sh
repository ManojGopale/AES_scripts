...
date
/usr/bin/time singularity exec --nv /extra/manojgopale/AES_data/dockerImage/ocelote_keras-2.2.4+tensorflow-1.13.1_gpu-cp35-cuda10.0-cudnn7.5.sif python3.5 /extra/manojgopale/AES_data/keyPrediction_scripts/crossConfigAnalysis.py -m ${modelConfig} -t ${testConfig} --trainSize ${trainSize} --modelDir ${modelDir} --testDir ${testDir} --modelName ${modelName} --trainFlag ${trainFlag} --devFlag ${devFlag} --testFlag ${testFlag}
date
