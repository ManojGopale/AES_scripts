...
class Classifier:
	def __init__(self, resultDir: str, modelName: str, x_train, y_train_oh, x_dev, y_dev_oh, x_test, y_test_oh):
		...
		self.model.add(Dense(500, input_shape=(1361,)))
	...
	def saveModel(self):
		""" Save the model
		"""
		saveStr = self.resultDir + '/' + self.modelName +'_3HLw_500_500_256_noDrop_' + str(self.history.epoch[-1]+1) + 'epochs_' + str(dropOut).replace('.', 'p') + 'Dropout_' + '{0:.2f}'.format(self.model_score[1]*100).replace('.', 'p') + '_' + str(batchSize) + '.h5'
