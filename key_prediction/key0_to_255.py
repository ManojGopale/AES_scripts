...
runDir = "/extra/manojgopale/AES_data/config3p1_10ktraining/"
dataDir = runDir + "data/"
...
#
#logFile = resultDir + 'm8_newTrial_withHist.log'
#csv_logger = CSVLogger(logFile, append=True, separator="\t")
#
#earlyStop = EarlyStopping(monitor='val_loss', patience=5, mode='auto', verbose=1)
#
...
#
#saveStr = resultDir + 'm8_3HLw_500_500_256_noDrop_' + str(Epochs) + 'epochs_' + str(dropOut).replace('.', 'p') + 'Dropout_' + '{0:.2f}'.format(model8_score[1]*100).replace('.', 'p') + '.h5'
#model8.save(saveStr)
