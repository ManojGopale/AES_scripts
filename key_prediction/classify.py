...
class Classifier:
	...
	def saveModel(self):
		""" Save the model
		"""
		saveStr = self.resultDir + '/' + self.modelName +'_3HLw_500_500_256_noDrop_' + str(self.history.epoch[-1]+1) + 'epochs_' + str(dropOut).replace('.', 'p') + 'Dropout_' + '{0:.2f}'.format(self.model_score[1]*100).replace('.', 'p') + '.h5'
