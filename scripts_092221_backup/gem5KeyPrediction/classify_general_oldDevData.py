...
def getData(dataPath, configName, trainSize, trainFlag, devFlag, testFlag):
	...
	for index, val in enumerate([1,0,3,2,7,5,6,4]):
		...
		## Dev data will be taken from older collected data.
		devStr   = "/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/python_15k_data/" + configName  + "/aesData_config9_Dev_" + str(val) + ".csv"
		...
		if (devFlag and os.path.isfile(devStr)):
		## get the data for each sub part
			print("Dev data processing for %s\n" %(devStr))
	...
	## Breakpoint for checking the devdata
	pdb.set_trace()
	...
	print("### Done loading dev data for %s from\n%s" %(configName, devStr))
	return (x_train, y_train_oh), (x_dev, y_dev_oh), (x_test, y_test_oh)

class Classifier:
	def __init__(self, resultDir: str, modelName: str, x_train, y_train_oh, x_dev, y_dev_oh, x_test, y_test_oh, hiddenLayer, actList, dropList, batchNorm, numPowerTraces, configName):
	...
	def saveModel(self):
		""" Save the model
		"""
		saveStr = self.resultDir + '/' + self.configName + '/' + self.modelName + '_' + str(len(self.hiddenLayer)) + 'HL_'  + str(self.history.epoch[-1]+1) + 'epochs_' + '{0:.2f}'.format(self.model_score[1]*100).replace('.', 'p') + '_acc_' + '.h5'
