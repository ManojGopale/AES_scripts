...
class StandardizeData:
	def __init__(self, x_train, y_train, x_dev, y_dev, x_test, y_test):
		"""Standardize data according to train, dev or test parameters.
		Also one-hot the y-values in this
		"""
	...
	def shuffleData(self, x_data, y_data):
		...
		print("\nStarted shuffling of data\nx_data[0]= %s\ny_data[0]= %s" %(x_data[0], y_data[0]))
		print(*x_data[0])
		print("\nx_data[12000]= %s\ny_data[12000]= %s" %(x_data[12000], y_data[12000]))
		print(*x_data[12000])
	...
	def stdByTrain(self, x_data):
		"""Standardize the data set with the parameters from the train dataset only
		"""
		...
		chunkSize = 25600
		chunkNum = int(len(x_data)/chunkSize)
		for chunkIndex in range(chunkNum):
			print("Train chunkIndx= %s, chunkNum = %s" %(chunkIndex, chunkNum))
			if(chunkIndex != chunkNum-1): 
				x_data[chunkIndex*chunkSize: (chunkIndex+1)*chunkSize] = (x_data[chunkIndex*chunkSize: (chunkIndex+1)*chunkSize]-self.x_train_mean)/self.x_train_std
			else:
				x_data[chunkIndex*chunkSize: ] = (x_data[chunkIndex*chunkSize: ] - self.x_train_mean)/self.x_train_std
		return x_data
			
	def stdByDev(self, x_data):
		"""Standardize the data set with the parameters from the dev dataset only
		"""
		self.x_dev_mean = self.x_dev.mean(axis=0)
		self.x_dev_std  = self.x_dev.mean(axis=0)
		...
		for chunkIndex in range(chunkNum):
			print("Train chunkIndx= %s, chunkNum = %s" %(chunkIndex, chunkNum))
			if(chunkIndex != chunkNum-1): 
				x_data[chunkIndex*chunkSize: (chunkIndex+1)*chunkSize] = (x_data[chunkIndex*chunkSize: (chunkIndex+1)*chunkSize]-self.x_dev_mean)/self.x_dev_std
			else:
				x_data[chunkIndex*chunkSize: ] = (x_data[chunkIndex*chunkSize: ] - self.x_dev_mean)/self.x_dev_std
		return x_data
			
	def stdByTest(self, x_data):
		"""Standardize the data set with the parameters from the test dataset only
		"""
		self.x_test_mean = self.x_test.mean(axis=0)
		self.x_test_std  = self.x_test.std(axis=0)
		...
		for chunkIndex in range(chunkNum):
			print("Train chunkIndx= %s, chunkNum = %s" %(chunkIndex, chunkNum))
			if(chunkIndex != chunkNum-1): 
				x_data[chunkIndex*chunkSize: (chunkIndex+1)*chunkSize] = (x_data[chunkIndex*chunkSize: (chunkIndex+1)*chunkSize]-self.x_test_mean)/self.x_test_std
			else:
				x_data[chunkIndex*chunkSize: ] = (x_data[chunkIndex*chunkSize: ] - self.x_test_mean)/self.x_test_std
		return x_data
			
	def stdByTrainDev(self, x_data):
		"""Standardize the data set with the parameters from the train and dev dataset
		"""
		## Concatenating train and dev
		x_full = np.concatenate((self.x_train, self.x_dev), axis=0)
		x_full_mean = x_full.mean(axis=0)
		x_full_std = x_full.std(axis=0)
		print("Started normalizing\n")
		chunkSize = 25600
		chunkNum = int(len(x_data)/chunkSize)
		for chunkIndex in range(chunkNum):
			print("Train chunkIndx= %s, chunkNum = %s" %(chunkIndex, chunkNum))
			if(chunkIndex != chunkNum-1): 
				x_data[chunkIndex*chunkSize: (chunkIndex+1)*chunkSize] = (x_data[chunkIndex*chunkSize: (chunkIndex+1)*chunkSize]-x_full_mean)/x_full_std
			else:
				x_data[chunkIndex*chunkSize: ] = (x_data[chunkIndex*chunkSize: ] - self.x_full_mean)/self.x_full_std
		return x_data
...
class Classifier:
	...
	def train(self, batchSize):
		...
		tensorboardDir=self.resultDir + "../log/tensorboardDir/" + self.modelName + "/"
		tensorboard = TensorBoard(log_dir=tensorboardDir, histogram_freq=1, batch_size=256, write_graph=False, write_grads=True, write_images=False, embeddings_freq=0, embeddings_layer_names=None, embeddings_metadata=None)
		...
		self.history = self.model.fit(self.x_train, self.y_train_oh, batch_size= batchSize, epochs=Epochs, verbose=1, shuffle= True, validation_data=(self.x_dev, self.y_dev_oh), callbacks=[csv_logger, earlyStop, tensorboard])
