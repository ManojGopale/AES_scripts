...
#####/usr/bin/time singularity exec --nv /xdisk/rlysecky/manojgopale/extra/fromBethard/AES_data/dockerImage/ocelote_keras-2.2.4+tensorflow-1.13.1_gpu-cp35-cuda10.0-cudnn7.5.sif  python3.5 /xdisk/rlysecky/manojgopale/extra/gem5KeyPrediction/scr/crossAnalysis_devSet.py --devDataConfig ${devDataConfig} --trainSize ${trainSize} --devSize ${devSize} --modelDir ${modelDir} --devDataDir ${devDataDir} --trainFlag ${trainFlag} --devFlag ${devFlag} --testFlag ${testFlag}  --inputTraces ${inputTraces} --typeOfStd ${typeOfStd}

######USed to test final models with test set
/usr/bin/time singularity exec --nv /xdisk/rlysecky/manojgopale/extra/fromBethard/AES_data/dockerImage/ocelote_keras-2.2.4+tensorflow-1.13.1_gpu-cp35-cuda10.0-cudnn7.5.sif  python3.5 /xdisk/rlysecky/manojgopale/extra/gem5KeyPrediction/scr/crossAnalysis_testSet.py --testDataConfig ${testDataConfig} --trainSize ${trainSize} --testSize ${testSize} --modelDir ${modelDir} --testDataDir ${testDataDir} --trainFlag ${trainFlag} --devFlag ${devFlag} --testFlag ${testFlag}  --inputTraces ${inputTraces} --typeOfStd ${typeOfStd}
date
