...
def getData(dataPath, trainSize, trainFlag, devFlag, testFlag):
	...
	for index, val in enumerate(range(0,256)):
		...
		## get the data for each sub part
			...
			x_train[trainSize*(index+1): (trainSize*(index+1)) + 4000, : ] = x_dev_inter[1000:5000, :]
			y_train[trainSize*(index+1): (trainSize*(index+1)) + 4000, 0] = y_dev_inter.reshape(5000, 1)[devSize:5000, 0]
