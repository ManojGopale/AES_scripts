...
def getData(dataPath, moreDataPath, trainSize, trainFlag, devFlag, testFlag):
	...
	for index, val in enumerate(range(0,256)):
		...
		if (trainFlag):
			...
			## Adding 1000 more data
			x_train_inter_more, y_train_inter_more = process_inputs(moreTrainStr)
			x_train[trainSize*(index+1): (trainSize*(index+1)) + 1000, : ] = x_train_inter_more[0:1000, :]
			y_train[trainSize*(index+1): (trainSize*(index+1)) + 1000, 0] = y_train_inter_more.reshape(9000,1)[0:1000, 0]
		...
		## get the data for each sub part
			...
			x_train[trainSize*(index+1)+1000: (trainSize*(index+1)) + 5000, : ] = x_dev_inter[1000:5000, :]
			y_train[trainSize*(index+1)+1000: (trainSize*(index+1)) + 5000, 0] = y_dev_inter.reshape(5000, 1)[devSize:5000, 0]
