...
matlab -r 'qResult_folders = ["64k_associate8_0",  "64k_associate8_1", "64k_associate8_2"];outputPath = "/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/config3p4/";generateTrace(qResult_folders, outputPath, 111, 120);exit;'
...
#######matlab -r "try;0=str2double(getenv('manoj_0'));15=str2double(getenv('manoj_15'));configName=string(getenv('configName'));fprintf('0=%d 15= %d, configName=%s',0, 15, configName);qResult_folders = [string('l24m_l1associate8_0'), 'l24m_l1associate8_1', 'l24m_l1associate8_2'];outputPath = '/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/' + configName';fprintf('\noutputPath=%s\n', outputPath);parfor key=0:15;generateTrace(qResult_folders, outputPath, key, key) ; end; catch;end;exit;"
date
