...
matlab -r 'qResult_folders = ["64k_associate8_0",  "64k_associate8_1", "64k_associate8_2"];outputPath = "/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/config3p4/";generateTrace(qResult_folders, outputPath, 166, 180);exit;'
...
#######matlab -r "try;166=str2double(getenv('manoj_166'));180=str2double(getenv('manoj_180'));configName=string(getenv('configName'));fprintf('166=%d 180= %d, configName=%s',166, 180, configName);qResult_folders = [string('l24m_l1associate8_0'), 'l24m_l1associate8_1', 'l24m_l1associate8_2'];outputPath = '/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/' + configName';fprintf('\noutputPath=%s\n', outputPath);parfor key=166:180;generateTrace(qResult_folders, outputPath, key, key) ; end; catch;end;exit;"
date
