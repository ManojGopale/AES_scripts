...
matlab -r 'qResult_folders = ["64k_associate8_0",  "64k_associate8_1", "64k_associate8_2"];outputPath = "/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/config3p4/";generateTrace(qResult_folders, outputPath, 241, 255);exit;'
...
#######matlab -r "try;241=str2double(getenv('manoj_241'));255=str2double(getenv('manoj_255'));configName=string(getenv('configName'));fprintf('241=%d 255= %d, configName=%s',241, 255, configName);qResult_folders = [string('l24m_l1associate8_0'), 'l24m_l1associate8_1', 'l24m_l1associate8_2'];outputPath = '/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/' + configName';fprintf('\noutputPath=%s\n', outputPath);parfor key=241:255;generateTrace(qResult_folders, outputPath, key, key) ; end; catch;end;exit;"
date
