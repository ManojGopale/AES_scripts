...
matlab -r 'qResult_folders = ["64k_associate8_0",  "64k_associate8_1", "64k_associate8_2"];outputPath = "/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/config3p4/";generateTrace(qResult_folders, outputPath, 211, 225);exit;'
...
#######matlab -r "try;211=str2double(getenv('manoj_211'));225=str2double(getenv('manoj_225'));configName=string(getenv('configName'));fprintf('211=%d 225= %d, configName=%s',211, 225, configName);qResult_folders = [string('l24m_l1associate8_0'), 'l24m_l1associate8_1', 'l24m_l1associate8_2'];outputPath = '/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/' + configName';fprintf('\noutputPath=%s\n', outputPath);parfor key=211:225;generateTrace(qResult_folders, outputPath, key, key) ; end; catch;end;exit;"
date
