...
matlab -r 'qResult_folders = ["64k_associate8_0",  "64k_associate8_1", "64k_associate8_2"];outputPath = "/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/config3p4/";generateTrace(qResult_folders, outputPath, 226, 240);exit;'
...
#######matlab -r "try;226=str2double(getenv('manoj_226'));240=str2double(getenv('manoj_240'));configName=string(getenv('configName'));fprintf('226=%d 240= %d, configName=%s',226, 240, configName);qResult_folders = [string('l24m_l1associate8_0'), 'l24m_l1associate8_1', 'l24m_l1associate8_2'];outputPath = '/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/' + configName';fprintf('\noutputPath=%s\n', outputPath);parfor key=226:240;generateTrace(qResult_folders, outputPath, key, key) ; end; catch;end;exit;"
date
