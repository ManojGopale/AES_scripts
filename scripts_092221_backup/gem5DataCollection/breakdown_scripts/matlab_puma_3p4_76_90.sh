...
matlab -r 'qResult_folders = ["64k_associate8_0",  "64k_associate8_1", "64k_associate8_2"];outputPath = "/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/config3p4/";generateTrace(qResult_folders, outputPath, 76, 90);exit;'
...
#######matlab -r "try;76=str2double(getenv('manoj_76'));90=str2double(getenv('manoj_90'));configName=string(getenv('configName'));fprintf('76=%d 90= %d, configName=%s',76, 90, configName);qResult_folders = [string('l24m_l1associate8_0'), 'l24m_l1associate8_1', 'l24m_l1associate8_2'];outputPath = '/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/' + configName';fprintf('\noutputPath=%s\n', outputPath);parfor key=76:90;generateTrace(qResult_folders, outputPath, key, key) ; end; catch;end;exit;"
date
