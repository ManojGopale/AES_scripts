...
matlab -r 'qResult_folders = ["64k_associate8_0",  "64k_associate8_1", "64k_associate8_2"];outputPath = "/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/config3p4/";generateTrace(qResult_folders, outputPath, 196, 210);exit;'
...
#######matlab -r "try;196=str2double(getenv('manoj_196'));210=str2double(getenv('manoj_210'));configName=string(getenv('configName'));fprintf('196=%d 210= %d, configName=%s',196, 210, configName);qResult_folders = [string('l24m_l1associate8_0'), 'l24m_l1associate8_1', 'l24m_l1associate8_2'];outputPath = '/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/' + configName';fprintf('\noutputPath=%s\n', outputPath);parfor key=196:210;generateTrace(qResult_folders, outputPath, key, key) ; end; catch;end;exit;"
date
