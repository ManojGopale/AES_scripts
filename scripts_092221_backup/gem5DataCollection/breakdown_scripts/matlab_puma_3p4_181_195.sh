...
mkdir -p /xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/config3p4

matlab -r 'qResult_folders = ["64k_associate8_0",  "64k_associate8_1", "64k_associate8_2"];outputPath = "/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/config3p4/";generateTrace(qResult_folders, outputPath, 181, 195);exit;'
...
#######matlab -r "try;181=str2double(getenv('manoj_181'));195=str2double(getenv('manoj_195'));configName=string(getenv('configName'));fprintf('181=%d 195= %d, configName=%s',181, 195, configName);qResult_folders = [string('l24m_l1associate8_0'), 'l24m_l1associate8_1', 'l24m_l1associate8_2'];outputPath = '/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/' + configName';fprintf('\noutputPath=%s\n', outputPath);parfor key=181:195;generateTrace(qResult_folders, outputPath, key, key) ; end; catch;end;exit;"
date
