...
matlab -r 'qResult_folders = ["64k_associate8_0",  "64k_associate8_1", "64k_associate8_2"];outputPath = "/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/config3p4/";generateTrace(qResult_folders, outputPath, 121, 135);exit;'
...
#######matlab -r "try;121=str2double(getenv('manoj_121'));135=str2double(getenv('manoj_135'));configName=string(getenv('configName'));fprintf('121=%d 135= %d, configName=%s',121, 135, configName);qResult_folders = [string('l24m_l1associate8_0'), 'l24m_l1associate8_1', 'l24m_l1associate8_2'];outputPath = '/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/' + configName';fprintf('\noutputPath=%s\n', outputPath);parfor key=121:135;generateTrace(qResult_folders, outputPath, key, key) ; end; catch;end;exit;"
date
