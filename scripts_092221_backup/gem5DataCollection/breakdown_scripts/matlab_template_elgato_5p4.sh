...
matlab -r 'qResult_folders = ["dram_l24m_l1associate8_0",  "dram_l24m_l1associate8_1", "dram_l24m_l1associate8_2"];outputPath = "/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/config5p4/";generateTrace(qResult_folders, outputPath, startKey, endKey);exit;'

##Mexport manoj_startKey=${startKey}
##Mexport manoj_endKey=${endKey}
##Mexport manoj_configName=${configName}
##Mmatlab -r "try;startKey=str2double(getenv('manoj_startKey'));endKey=str2double(getenv('manoj_endKey'));configName=string(getenv('configName'));fprintf('startKey=%d endKey= %d, configName=%s',startKey, endKey, configName);qResult_folders = [string('l24m_l1associate8_0'), 'l24m_l1associate8_1', 'l24m_l1associate8_2'];outputPath = '/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/' + configName';fprintf('\noutputPath=%s\n', outputPath);parfor key=startKey:endKey; generateTrace(qResult_folders, outputPath, key, key);end;catch;end;exit;"
date
