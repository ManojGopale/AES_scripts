...
matlab -r 'qResult_folders = ["64k_associate8_0",  "64k_associate8_1", "64k_associate8_2"];outputPath = "/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/config3p4/";generateTrace(qResult_folders, outputPath, 16, 30);exit;'
...
#######matlab -r "try;16=str2double(getenv('manoj_16'));30=str2double(getenv('manoj_30'));configName=string(getenv('configName'));fprintf('16=%d 30= %d, configName=%s',16, 30, configName);qResult_folders = [string('l24m_l1associate8_0'), 'l24m_l1associate8_1', 'l24m_l1associate8_2'];outputPath = '/xdisk/rlysecky/manojgopale/xdisk/gem5DataCollection/matResult/' + configName';fprintf('\noutputPath=%s\n', outputPath);parfor key=16:30;generateTrace(qResult_folders, outputPath, key, key) ; end; catch;end;exit;"
date
