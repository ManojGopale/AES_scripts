...
parser.add_option('--drop5',
									action = 'store', type='float', dest='drop5', default = 0.2)
parser.add_option('--drop6',
									action = 'store', type='float', dest='drop6', default = 0.2)
parser.add_option('--drop7',
									action = 'store', type='float', dest='drop7', default = 0.2)
parser.add_option('--drop8',
									action = 'store', type='float', dest='drop8', default = 0.2)
...
trainData, devData, testData = classify_8HL.getData(dataDir, trainSize, trainFlag, devFlag, testFlag)
...
t0_time = time.time()
classifier = classify_8HL.Classifier(resultDir, modelName, x_train, y_train_oh, x_dev, y_dev_oh, x_test, y_test_oh, drop1, drop2, drop3, drop4, drop5, drop6, drop7, drop8)
t1_time = time.time()
