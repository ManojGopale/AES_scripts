...
class Classifier:
	...
	def evaluate(self):
		...
		outputFile = self.resultDir + '/' + self.modelName +'_6HLw_1000_700_500_500_300_300_' + str(self.history.epoch[-1]+1) + 'epochs_' + 'Dropout_' + str(self.drop1).replace('.', 'p') + '_' + str(self.drop2).replace('.', 'p') + '_' + str(self.drop3).replace('.', 'p')  + '_' +  "_outputPredict.csv" 
	...
	def saveModel(self):
		""" Save the model
		"""
		saveStr = self.resultDir + '/' + self.modelName +'_6HLw_1000_700_500_500_300_300_' + str(self.history.epoch[-1]+1) + 'epochs_' + 'Dropout_' + str(self.drop1).replace('.', 'p') + '_' + str(self.drop2).replace('.', 'p') + '_' + str(self.drop3).replace('.', 'p')  + '_' + '{0:.2f}'.format(self.model_score[1]*100).replace('.', 'p') + '.h5'
