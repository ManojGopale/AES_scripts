...
class Classifier:
	def __init__(self, resultDir: str, modelName: str, x_train, y_train_oh, x_dev, y_dev_oh, x_test, y_test_oh, drop1, drop2, drop3, drop4, drop5):
	...
	def evaluate(self):
		...
		outputFile = self.resultDir + '/' + self.modelName +'_5HLw_1000_700_500_500_300' + str(self.history.epoch[-1]+1) + 'epochs_' + 'Dropout_' + str(self.drop1).replace('.', 'p') + '_' + str(self.drop2).replace('.', 'p') + '_' + str(self.drop3).replace('.', 'p')  + '_' + str(self.drop4).replace('.', 'p')  +'_' + str(self.drop5).replace('.', 'p')  +  "_outputPredict.csv" 
	...
	def saveModel(self):
		""" Save the model
		"""
		saveStr = self.resultDir + '/' + self.modelName +'_5HLw_1000_700_500_500_300' + str(self.history.epoch[-1]+1) + 'epochs_' + 'Dropout_' + str(self.drop1).replace('.', 'p') + '_' + str(self.drop2).replace('.', 'p') + '_' + str(self.drop3).replace('.', 'p')  + '_' + str(self.drop4).replace('.', 'p')  + '_' + str(self.drop5).replace('.', 'p')  + '_' + '{0:.2f}'.format(self.model_score[1]*100).replace('.', 'p') + '.h5'
