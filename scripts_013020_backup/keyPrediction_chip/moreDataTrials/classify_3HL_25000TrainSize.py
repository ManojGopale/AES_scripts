...
def getData(dataPath, moreDataPath, trainSize, trainFlag, devFlag, testFlag):
	...
	for index, val in enumerate(range(0,256)):
		...
		if (trainFlag):
			...
			x_train[trainSize*(index) + 15000: (trainSize*(index) + 15000) + 6000, : ] = x_train_inter_more[0:6000, :]
			y_train[trainSize*(index) + 15000: (trainSize*(index) + 15000) + 6000, 0] = y_train_inter_more.reshape(9000,1)[0:6000, 0]
		...
		## get the data for each sub part
			...
			x_train[trainSize*(index) + 15000 +6000: (trainSize*(index) + 15000) + 10000, : ] = x_dev_inter[1000:5000, :]
			y_train[trainSize*(index) + 15000 +6000: (trainSize*(index) + 15000) + 10000, 0] = y_dev_inter.reshape(5000, 1)[devSize:5000, 0]
