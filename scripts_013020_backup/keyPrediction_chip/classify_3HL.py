...
class Classifier:
	def __init__(self, resultDir: str, modelName: str, x_train, y_train_oh, x_dev, y_dev_oh, x_test, y_test_oh, drop1, drop2, drop3):
		...
		self.model.add(Dense(2000, activation='relu', input_shape=(1500,)))
	...
	def train(self, batchSize):
		...
		earlyStop = EarlyStopping(monitor='val_categorical_accuracy', patience=50, mode='auto', verbose=1, restore_best_weights=True)
	...
	def saveModel(self):
		""" Save the model
		"""
		saveStr = self.resultDir + '/' + self.modelName +'_3HLw_2000_1000_1500_' + str(self.history.epoch[-1]+1) + 'epochs_' + 'Dropout_' + str(self.drop1).replace('.', 'p') + '_' + str(self.drop2).replace('.', 'p') + '_' + str(self.drop3).replace('.', 'p')  + '_' + '{0:.2f}'.format(self.model_score[1]*100).replace('.', 'p') + '.h5'
