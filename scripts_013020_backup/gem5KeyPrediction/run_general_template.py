...
trainData, devData, testData = classify_general.getData(dataDir, configName, trainSize, trainFlag, devFlag, testFlag)
...
##MnumHiddenLayers = numAllHiddenLayers[np.random.random_integers(0, len(numAllHiddenLayers)-1)]
numHiddenLayers = 4
...
##MhiddenLayer = np.random.randint(1000, 10000, numHiddenLayers) ##Runs 130-170
...
t0_time = time.time()
classifier = classify_general.Classifier(resultDir, modelName, x_train, y_train_oh, x_dev, y_dev_oh, x_test, y_test_oh, hiddenLayer, actList, dropList, batchNorm, numPowerTraces, configName)
t1_time = time.time()
