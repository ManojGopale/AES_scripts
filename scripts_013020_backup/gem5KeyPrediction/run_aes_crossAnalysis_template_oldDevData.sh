...
date
/usr/bin/time singularity exec --nv /xdisk/rlysecky/manojgopale/extra/fromBethard/AES_data/dockerImage/ocelote_keras-2.2.4+tensorflow-1.13.1_gpu-cp35-cuda10.0-cudnn7.5.sif  python3.5 /xdisk/rlysecky/manojgopale/extra/gem5KeyPrediction/scr/crossAnalysis_oldDevData.py --devDataConfig ${devDataConfig} --trainSize ${trainSize} --modelDir ${modelDir} --devDataDir ${devDataDir} --trainFlag ${trainFlag} --devFlag ${devFlag} --testFlag ${testFlag} --count ${count}
date
